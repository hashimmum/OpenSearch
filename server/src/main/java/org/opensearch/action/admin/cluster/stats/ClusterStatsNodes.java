...
public class ClusterStatsNodes implements ToXContentFragment {
    ...
    ClusterStatsNodes(List<ClusterStatsNodeResponse> nodeResponses) {
        this(SubMetrics.allSubMetrics(Metric.NODES), nodeResponses);
    }

    ClusterStatsNodes(Set<String> requestedMetrics, List<ClusterStatsNodeResponse> nodeResponses) {
        this.versions = new HashSet<>();
        boolean isFSInfoRequested = SubMetrics.FS.containedIn(requestedMetrics);
        boolean isPluginsInfoRequested = SubMetrics.PLUGINS.containedIn(requestedMetrics);
        ...
        this.os = SubMetrics.OS.containedIn(requestedMetrics) ? new OsStats(nodeInfos, nodesStats) : null;
        this.process = SubMetrics.PROCESS.containedIn(requestedMetrics) ? new ProcessStats(nodesStats) : null;
        this.jvm = SubMetrics.JVM.containedIn(requestedMetrics) ? new JvmStats(nodeInfos, nodesStats) : null;
        this.networkTypes = SubMetrics.NETWORK_TYPES.containedIn(requestedMetrics) ? new NetworkTypes(nodeInfos) : null;
        this.discoveryTypes = SubMetrics.DISCOVERY_TYPES.containedIn(requestedMetrics) ? new DiscoveryTypes(nodeInfos) : null;
        this.packagingTypes = SubMetrics.PACKAGING_TYPES.containedIn(requestedMetrics) ? new PackagingTypes(nodeInfos) : null;
        this.ingestStats = SubMetrics.INGEST.containedIn(requestedMetrics) ? new IngestStats(nodesStats) : null;
    }
